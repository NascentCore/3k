...
class Internal_Add_Tensor_Scalar(InternalMetaOperator):
    ...
    def generate_op(self, splited_input_signature: List[InternalMetaVariable], target: tvm.target.Target):
        ...
        def _internal_add_t_s(shape,dtype,prefix):
        ...
        task = auto_scheduler.SearchTask(_internal_add_t_s, args=(shape, dtype, f'{self.op_name}_{splited_input_signature[0].id}'), target=target)
...
class Internal_Add_Tensor_Tensor(InternalMetaOperator):
    def __init__(self, op_id: int, input_signature: List[InternalMetaVariable]):
        super().__init__(op_id, input_signature)
        self.op_name = 'add_t_t'
    ...
    def generate_op(self, splited_input_signature: List[InternalMetaVariable], target: tvm.target.Target):
        @auto_scheduler.register_workload
        def _internal_add_t_s(shape0, shape1,dtype,prefix):
        ...
        dtype = splited_input_signature[0].dtype
        task = auto_scheduler.SearchTask(_internal_add_t_s, args=(shape0, shape1, dtype, f'{self.op_name}_{splited_input_signature[0].id}'), target=target)
        tune_option = auto_scheduler.TuningOptions (
            num_measure_trials=20,
            measure_callbacks=[auto_scheduler.RecordToFile(log_file)],
            verbose=2
...
class Internal_Add_Scalar_Scalar(InternalMetaOperator):
    ...
    def generate_op(self, splited_input_signature: List[InternalMetaVariable], target: tvm.target.Target):
        @auto_scheduler.register_workload
        def _internal_add_t_s(dtype,prefix):
        ...
        log_file = f'tmp_operator_search_logs/{self.op_name}_{splited_input_signature[0].id}.log'
        dtype = splited_input_signature[0].dtype
        task = auto_scheduler.SearchTask(_internal_add_t_s, args=(dtype, f'{self.op_name}_{splited_input_signature[0].id}'), target=target)
        tune_option = auto_scheduler.TuningOptions (
...
class Internal_Add:
    ...
    def get_input_sig_from_meta_node(meta_node: MetaNode):
        ...
        if len(meta_inputs[1].shape) == 0:
            ...
            input_signature = [InternalMetaVariable(None, InternalType.Tensor, meta_inputs[0].dtype, meta_inputs[0].shape, None, None),
                               InternalMetaVariable(None, InternalType.Tensor, meta_inputs[0].dtype, meta_inputs[1].shape, None, None),]
        return input_signature
    
    @staticmethod
    def get_dispatched(id: int, input_signatrue: List[InternalMetaVariable]):
        if input_signatrue[0].type == InternalType.Scalar:
            return Internal_Add_Scalar_Scalar(id, input_signatrue)
        elif input_signatrue[1].type == InternalType.Scalar:
            return Internal_Add_Tensor_Scalar(id, input_signatrue)
        else:
            return Internal_Add_Tensor_Tensor(id, input_signatrue)
