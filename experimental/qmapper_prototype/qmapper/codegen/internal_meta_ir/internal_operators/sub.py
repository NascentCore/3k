...
class Internal_Sub_Tensor_Scalar(InternalMetaOperator):
    ...
    def generate_op(self, splited_input_signature: List[InternalMetaVariable], target: tvm.target.Target):
        ...
        def _internal_sub_t_s(shape,dtype,prefix):
        ...
        task = auto_scheduler.SearchTask(_internal_sub_t_s, args=(shape, dtype, f'{self.op_name}_{splited_input_signature[0].id}'), target=target)
...
class Internal_Sub_Tensor_Tensor(InternalMetaOperator):
    ...
    def generate_op(self, splited_input_signature: List[InternalMetaVariable], target: tvm.target.Target):
        @auto_scheduler.register_workload
        def _internal_sub_t_s(shape0, shape1,dtype,prefix):
        ...
        dtype = splited_input_signature[0].dtype
        task = auto_scheduler.SearchTask(_internal_sub_t_s, args=(shape0, shape1, dtype, f'{self.op_name}_{splited_input_signature[0].id}'), target=target)
        tune_option = auto_scheduler.TuningOptions (
            num_measure_trials=20,
            measure_callbacks=[auto_scheduler.RecordToFile(log_file)],
            verbose=2
...
class Internal_Sub_Scalar_Scalar(InternalMetaOperator):
    def __init__(self, op_id: int, input_signature: List[InternalMetaVariable]):
        super().__init__(op_id, input_signature)
        assert input_signature[0].shape == input_signature[1].shape, "Two shape must be identical"
        self.op_name = 'sub_s_s'
    ...
    def generate_op(self, splited_input_signature: List[InternalMetaVariable], target: tvm.target.Target):
        @auto_scheduler.register_workload
        def _internal_sub_t_s(dtype,prefix):
        ...
        log_file = f'tmp_operator_search_logs/{self.op_name}_{splited_input_signature[0].id}.log'
        dtype = splited_input_signature[0].dtype
        task = auto_scheduler.SearchTask(_internal_sub_t_s, args=(dtype, f'{self.op_name}_{splited_input_signature[0].id}'), target=target)
        tune_option = auto_scheduler.TuningOptions (
...
class Internal_Sub:
    ...
    def get_dispatched(id: int, input_signatrue: List[InternalMetaVariable]):
        if input_signatrue[0].type == InternalType.Scalar:
            return Internal_Sub_Scalar_Scalar(id, input_signatrue)
        elif input_signatrue[1].type == InternalType.Scalar:
            return Internal_Sub_Tensor_Scalar(id, input_signatrue)
        else:
            return Internal_Sub_Tensor_Tensor(id, input_signatrue)
