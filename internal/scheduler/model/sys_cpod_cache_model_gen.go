...
type (
	...
	SysCpodCache struct {
		...
		NewUserId         sql.NullString `db:"new_user_id"`         // 用户ID
...
func (m *defaultSysCpodCacheModel) Insert(ctx context.Context, data *SysCpodCache) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.CpodId, data.CpodVersion, data.DataType, data.DataName, data.DataId, data.DataSize, data.DataSource, data.Public, data.UserId, data.NewUserId, data.Template, data.FinetuneGpuCount, data.InferenceGpuCount)
	return ret, err
}
...
func (m *defaultSysCpodCacheModel) Update(ctx context.Context, data *SysCpodCache) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, data.CpodId, data.CpodVersion, data.DataType, data.DataName, data.DataId, data.DataSize, data.DataSource, data.Public, data.UserId, data.NewUserId, data.Template, data.FinetuneGpuCount, data.InferenceGpuCount, data.Id)
	return err
}
