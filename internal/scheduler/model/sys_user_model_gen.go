...
var (
	...
	sysUserRowsExpectAutoSet   = strings.Join(stringx.Remove(sysUserFieldNames, "`user_id`", "`create_at`", "`create_time`", "`created_at`", "`update_at`", "`update_time`", "`updated_at`"), ",")
...
type (
	...
	SysUser struct {
		...
		IsAdmin      byte           `db:"is_admin"`       // 是否为admin账号
...
func (m *defaultSysUserModel) Insert(ctx context.Context, data *SysUser) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.Username, data.NickName, data.Gender, data.Phone, data.Email, data.AvatarName, data.AvatarPath, data.Password, data.IsAdmin, data.Enabled, data.CreateBy, data.UpdateBy, data.PwdResetTime, data.UserType, data.CompanyName, data.CompanyPhone, data.CompanyOther, data.CompanyId)
	return ret, err
}
