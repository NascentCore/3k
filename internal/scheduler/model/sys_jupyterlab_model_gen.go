...
func (m *defaultSysJupyterlabModel) Insert(ctx context.Context, data *SysJupyterlab) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.JobName, data.UserId, data.CpodId, data.Status, data.InstanceName, data.GpuCount, data.GpuProd, data.CpuCount, data.MemCount, data.DataVolumeSize, data.ModelId, data.ModelName, data.ModelPath, data.StartTime, data.EndTime)
	return ret, err
}
...
func (m *defaultSysJupyterlabModel) Update(ctx context.Context, newData *SysJupyterlab) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, newData.JobName, newData.UserId, newData.CpodId, newData.Status, newData.InstanceName, newData.GpuCount, newData.GpuProd, newData.CpuCount, newData.MemCount, newData.DataVolumeSize, newData.ModelId, newData.ModelName, newData.ModelPath, newData.StartTime, newData.EndTime, newData.Id)
	return err
}
