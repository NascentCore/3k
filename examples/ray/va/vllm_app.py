    ...
    async def create_chat_completion(
        ...
        if not self.openai_serving_chat:
            ...
            model_path = BaseModelPath(
                name=self.engine_args.model,  # 使用模型名称作为 name
                model_path=self.engine_args.model  # 完整的模型路径
            )
            
            self.openai_serving_chat = OpenAIServingChat(
                self.engine,
                model_config,
                base_model_paths=[model_path],  # 使用 ModelPath 对象
...
def build_app(cli_args: Dict[str, str]) -> serve.Application:
    """构建 Serve 应用"""
    # 使用类变量或缓存存储解析结果
    if not hasattr(build_app, '_parsed_args'):
        build_app._parsed_args = parse_vllm_args(cli_args)
