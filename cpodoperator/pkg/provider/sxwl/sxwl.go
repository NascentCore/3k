...
func (s *sxwl) GetAssignedJobList() ([]PortalTrainningJob, []PortalInferenceJob, []PortalJupyterLabJob, []PortalYAMLResource, []UserID, error) {
	...
	for _, v := range res.YAMLResources {
		exist := false
		for _, userID := range userIDs {
			if userID == UserID(v.UserID) {
				exist = true
	...
	return res.TrainningJobs, res.InferenceJobs, res.JupyterLabJobs, res.YAMLResources, userIDs, nil
}
